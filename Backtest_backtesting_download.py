...
#Cryto data and analytics: https://www.cryptodatadownload.com/data/binance/
...
data_1m = pd.read_csv('Backtest_data\Binance_BTCUSDT_minute.csv', usecols=['date', 'open', 'high', 'low', 'close', 'Volume USDT'])
...
supertrend_30m = set_data_param(data_1m, 30, start_date)
...
def backtest_(df):
    bt = Backtest(df, supertrend_signal, cash=10000, commission=.001, exclusive_orders=True)
    stats = bt.run()
    bt.plot()
    return stats
...
stat['15m'] = stats_15m
stat['30m'] = stats_30m
print("Outputing {} backtest data to csv.".format(Coin))

stat.to_csv(r'C:\Users\q3604\Desktop\Algo Trading\CCXT_BOT\Backtest_stats\{}_stat.csv'.format(Coin.replace("/", "_")))  #The dir cannot include /

#Further to dev
# 1. Output file naming with timeperiods
# stage two: maximum some indicators, e.g. sharpe ratio


#change the datetime to unixtimestamp
# data_1m['timestamp'] = data_1m.index.view('int64') / 1000000
# data_1m['timestamp'] = data_1m['timestamp'].values.astype('datetime64[ms]')

# print(supertrend_1m[supertrend_1m.index.duplicated()])  #check for duplicates in timestamps